...
**Context.** The brand ships a 10 s cover clip (`portada.mp4`, 1920×1080, H.264)
that the hero has to carry "transparently". The file has **no alpha channel** —
sampling frames off-screen put its background at roughly `#131313` with only 1%
of pixels below luminance 18. Played straight it renders as an opaque rectangle
that fights the headline for contrast.

**Decision.** Composite it with `mix-blend-mode: screen` at `opacity-30`, over a
radial veil that fades its edges into `--background`. Screen maps black to fully
transparent, so on the `#050505` canvas the clip's dark ground contributes
almost nothing while the glowing UI in the footage survives — a true cut-out
rather than a video box. No alpha-capable re-encode (WebM/VP9 or HEVC) is
needed, which also avoids shipping two formats.

**Consequences.**
- The treatment depends on the source being **dark-on-black**. A clip with a
  light background would need `multiply` instead, or a real alpha encode.
- Blending is GPU-cheap here because the video is `position: absolute` and never
  composited against scrolling content.
- The asset is 13 MB with `moov` at the end of the container, so playback waits
  on the full download. Marked as follow-up work: re-encode with `-movflags
  +faststart` and a tighter bitrate before launch.

---

## ADR-0018 — One `TextEngine` per headline line; `seo={false}` on gradient lines

- **Status:** Accepted
- **Date:** 2026-07-29

**Context.** Building the landing headlines surfaced two failure modes in
[[text-engine]] that the note did not yet spell out.

1. A single engine with `<br>` between the lines rendered **one line**. The
   engine's container is a wrapping flex row and its children are word slots, so
   a `<br>` is not a break it can act on — the visible break only appeared when
   the container happened to be narrow enough to wrap naturally.
2. Putting `bg-clip-text text-transparent` on the container made the engine's
   hidden plain-text SEO copy **paint on top of** the animated slots, showing as
   doubled, offset glyphs.

**Decision.** Each headline line is its own `TextEngine tag="span"` inside the
semantic heading element, with `delayIn` staggering the lines. Lines that carry
the brand gradient additionally set `seo={false}`.

**Consequences.**
- Line breaks are deliberate and independent of container width.
- Dropping the SEO copy costs nothing for crawlers: the animated slots contain
  the real text in the DOM. Only apply it to gradient-clipped lines.
- Codified in [[components/ui]] and applied in `views/home/hero-headline.tsx`,
  `views/home/cta-headline.tsx` and `components/ui/section-heading.tsx`.
